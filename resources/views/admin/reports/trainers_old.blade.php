                ...
                <tbody>
                    @forelse ($trainers as $trainer)
                        <tr>
                            ...
                            <td class="font-weight-bold ">{{ number_format($commission->where('trainer_id', $trainer->id)->first()['totalInvoices']) . ' SAR' }}</td>
                            @php
                                $pre_com = $pre_commission->where('trainer_id', $trainer->id)->first();
                            @endphp
                            <td class="font-weight-bold">
                                {{ round($commission->where('trainer_id', $trainer->id)->first()['total']) }} SAR
                            </td>
                            <td class="font-weight-bold">
                            ...
                            <td class="font-weight-bold">
                                {{ $commission->where('trainer_id', $trainer->id)->first()['commission'] != 0 ? round(intval($commission->where('trainer_id', $trainer->id)->first()['commission'])) . ' SAR' : '0 SAR ' }}
                            </td>
                            <td class="font-weight-bold">
                                @if(isset($pre_com) && $pre_com != NULL) 
                                ...
                                @endif</td>
                            <td class="font-weight-bold">
                                {{ round(intval($commission->where('trainer_id', $trainer->id)->first()['commission'])) + (isset($pre_com['previous_months_commissions']) ? round($pre_com['previous_months_commissions']) : 0) }} SAR
                            </td>
                            <td>
